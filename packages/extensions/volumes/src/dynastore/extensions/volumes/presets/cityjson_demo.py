...
3D tile content is served at runtime by the volumes extension —
no offline tileset task is enqueued by this preset.
"""
...
class _GeoVolumesDemoPreset:
    ...
    Volumes tiles are served at runtime by the volumes extension —
    no offline task is enqueued.
    """

    name: ClassVar[str] = "geovolumes_demo"
    description: ClassVar[str] = (
        "Seed a demo-3d catalog with Den Haag LoD2 CityJSON buildings "
        "(2498 CityObjects, EPSG:7415). 3D Tiles served at runtime by the volumes extension."
    )
    ...
    async def revoke(
        ...
        # --- Catalog (only if we created it AND the collection was removed) ---
