...
// The page must load MapLibre GL JS (v5.x) and deck.gl before this script.
// Exposed as a window global (plain script, not an ES module) consistent with
// leaflet-map.js and ogc-browser.js conventions.
...
(function (global) {
   ...
   */
  function createMapLibreGlobe(containerId, opts) {
    opts = opts || {};
    ...
    var overlay = new deck.MapboxOverlay({ interleaved: true, layers: [] });
    map.addControl(overlay);

    return { map: map, overlay: overlay };
  }

  global.createMapLibreGlobe = createMapLibreGlobe;
})(window);
