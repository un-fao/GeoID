...
async def lookup_by_external_id_across_collections(
    catalog_id: str,
    external_id: str,
    collection_id: Optional[str] = None,
    limit: int = 10,
) -> List[Dict[str, Any]]:
    """Resolve ``external_id`` within a catalog, optionally narrowed to one
    collection (#1210).

    ``external_id`` is the tenant's own item id and is not a document id, so —
    unlike :func:`lookup_by_geoids` — it cannot be served by the private ES
    driver's id-fetch path (that driver has no field-query surface). Resolution
    therefore runs over PostgreSQL, the durability primary present in every
    catalog, via the per-collection :func:`lookup_by_external_id` primitive:

    * ``collection_id`` given → a single per-collection lookup.
    * ``collection_id`` omitted → enumerate the catalog's collections and
      resolve in each, aggregating up to ``limit`` rows. ``external_id`` is not
      globally unique, so more than one collection may match.

    Returns rows shaped like ``GeoidResult``.
    """
    if collection_id:
        return await lookup_by_external_id(
            catalog_id, collection_id, external_id, limit=limit,
        )
    ...
    if not catalogs:
        logger.warning(
            "lookup_by_external_id_across_collections: CatalogsProtocol not available"
        )
        return []
    ...
    while len(out) < limit:
        try:
            collections = await catalogs.list_collections(
                catalog_id, limit=batch, offset=offset,
            )
        except Exception as exc:
            logger.warning(
                "lookup_by_external_id_across_collections: list_collections failed "
                "for '%s': %s",
                catalog_id, exc,
            )
            break
        ...
        for col in collections:
            ...
            rows = await lookup_by_external_id(
                catalog_id, col_id, external_id, limit=limit - len(out),
            )
            out.extend(rows)
