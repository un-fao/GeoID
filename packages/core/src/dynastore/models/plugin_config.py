...
into this leaf breaks that cycle structurally — see #686.  Moving it to
``models/`` completes the layering fix: the import direction is now
models → tools (correct), not models → modules → models (cycle) — see
#1555.  The marker types live in the sibling leaf
``dynastore.models.mutability``.
"""
