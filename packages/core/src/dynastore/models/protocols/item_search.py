...
Backend-agnostic single-method contract for the STAC ``/search`` ES fast
path (and any future consumer that needs a ``filter``-free item lookup).
The protocol takes plain kwargs + returns a plain dataclass so that
``extensions/stac/search.py`` can dispatch via ``get_protocol`` without
importing search-extension models or service classes.

Closes the architectural debt called out in #234 / #231 (the two
