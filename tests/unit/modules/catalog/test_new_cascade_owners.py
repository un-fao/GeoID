...
- EsItemsPublicIndexOwner: describe_scope, cleanup_one HARD/SOFT/error.
- EsAssetIndexOwner: describe_scope, cleanup_one.
- EsItemsEnvelopeIndexOwner: describe_scope, cleanup_one.
...
# ---------------------------------------------------------------------------
# EsItemsPublicIndexOwner
# ---------------------------------------------------------------------------


class TestEsItemsPublicIndexOwner:
    def _make_owner(self):
        from dynastore.modules.storage.drivers.elasticsearch_cascade_owners import (
            EsItemsPublicIndexOwner,
        )
        return EsItemsPublicIndexOwner()

    _EXPECTED_INDEX = f"{_PREFIX}-{_CATALOG_ID}-items"

    def _patch_index_name(self):
        return patch(
            "dynastore.modules.storage.drivers.elasticsearch_cascade_owners._get_public_items_index_name",
            return_value=self._EXPECTED_INDEX,
        )

    def _patch_es_client(self, raises=None):
        client = MagicMock()
        if raises:
        ...
        return patch(
            "dynastore.modules.storage.drivers.elasticsearch_cascade_owners._get_es_client",
            return_value=client,
        ), client
    ...
    async def test_describe_scope_catalog_returns_one_ref(self) -> None:
        owner = self._make_owner()
        scope_ref = ScopeRef(scope=ResourceScope.CATALOG, catalog_id=_CATALOG_ID)
        with self._patch_index_name():
            refs = await owner.describe_scope(scope_ref, MagicMock())
        assert len(refs) == 1
        assert refs[0].locator == self._EXPECTED_INDEX
    ...
    @pytest.mark.asyncio
    async def test_cleanup_one_hard_deletes_index(self) -> None:
        owner = self._make_owner()
        ref = CleanupRef(
            kind="es_index", locator=self._EXPECTED_INDEX, owner_id="es_public.items_index",
        )
        patch_ctx, es = self._patch_es_client()
        with patch_ctx:
            outcome = await owner.cleanup_one(ref, CleanupMode.HARD)
    ...
    async def test_cleanup_one_soft_returns_done(self) -> None:
        owner = self._make_owner()
        ref = CleanupRef(kind="es_index", locator=self._EXPECTED_INDEX, owner_id="es_public.items_index")
        patch_ctx, es = self._patch_es_client()
        with patch_ctx:
            outcome = await owner.cleanup_one(ref, CleanupMode.SOFT)
    ...
    @pytest.mark.asyncio
    async def test_cleanup_one_hard_exception_returns_retry(self) -> None:
        owner = self._make_owner()
        ref = CleanupRef(kind="es_index", locator=self._EXPECTED_INDEX, owner_id="es_public.items_index")
        patch_ctx, _ = self._patch_es_client(raises=OSError("network error"))
        with patch_ctx:
            outcome = await owner.cleanup_one(ref, CleanupMode.HARD)
        assert outcome == CleanupOutcome.RETRY
...
class TestEsAssetIndexOwner:
    def _make_owner(self):
        from dynastore.modules.storage.drivers.elasticsearch_cascade_owners import EsAssetIndexOwner
        return EsAssetIndexOwner()

    _EXPECTED_INDEX = f"{_PREFIX}-{_CATALOG_ID}-assets"

    def _patch_index_name(self):
        return patch(
            "dynastore.modules.storage.drivers.elasticsearch_cascade_owners._get_assets_index_name",
            return_value=self._EXPECTED_INDEX,
        )
    ...
    async def test_describe_scope_catalog_returns_one_ref(self) -> None:
        ...
        assert len(refs) == 1
        assert refs[0].locator == self._EXPECTED_INDEX
        assert refs[0].kind == "es_index"

    @pytest.mark.asyncio
    async def test_cleanup_one_hard_deletes_index(self) -> None:
        owner = self._make_owner()
        ref = CleanupRef(kind="es_index", locator=self._EXPECTED_INDEX, owner_id="es_public.asset_index")
        patch_ctx, es = self._patch_es_client()
        with patch_ctx:
            outcome = await owner.cleanup_one(ref, CleanupMode.HARD)
        es.indices.delete.assert_awaited_once()
    ...
    async def test_cleanup_one_hard_exception_returns_retry(self) -> None:
        owner = self._make_owner()
        ref = CleanupRef(kind="es_index", locator=self._EXPECTED_INDEX, owner_id="es_public.asset_index")
        patch_ctx, _ = self._patch_es_client(raises=RuntimeError("boom"))
        with patch_ctx:
            outcome = await owner.cleanup_one(ref, CleanupMode.HARD)
        assert outcome == CleanupOutcome.RETRY
...
# ---------------------------------------------------------------------------
# EsItemsEnvelopeIndexOwner
# ---------------------------------------------------------------------------


class TestEsItemsEnvelopeIndexOwner:
    def _make_owner(self):
        from dynastore.modules.storage.drivers.elasticsearch_envelope.cascade_owners import (
            EsItemsEnvelopeIndexOwner,
        )
        return EsItemsEnvelopeIndexOwner()

    _EXPECTED_INDEX = f"{_PREFIX}-{_CATALOG_ID}-envelope-items"

    def _patch_index_name(self):
        return patch(
            "dynastore.modules.storage.drivers.elasticsearch_envelope.cascade_owners._get_envelope_index_name",
            return_value=self._EXPECTED_INDEX,
        )
    ...
    def _patch_es_client(self, raises=None):
        ...
        if raises:
            ...
            client.indices.delete = AsyncMock(return_value=None)
        return patch(
            "dynastore.modules.storage.drivers.elasticsearch_envelope.cascade_owners._get_es_client",
            return_value=client,
        ), client
    ...
    async def test_cleanup_one_hard_deletes_index(self) -> None:
        owner = self._make_owner()
        ref = CleanupRef(kind="es_index", locator=self._EXPECTED_INDEX, owner_id="es_envelope.items_index")
        patch_ctx, es = self._patch_es_client()
        with patch_ctx:
            outcome = await owner.cleanup_one(ref, CleanupMode.HARD)
        es.indices.delete.assert_awaited_once()
    ...
    @pytest.mark.asyncio
    async def test_cleanup_one_soft_returns_done_no_delete(self) -> None:
        owner = self._make_owner()
        ref = CleanupRef(kind="es_index", locator=self._EXPECTED_INDEX, owner_id="es_envelope.items_index")
        patch_ctx, es = self._patch_es_client()
        with patch_ctx:
    ...
    async def test_cleanup_one_exception_returns_retry(self) -> None:
        owner = self._make_owner()
        ref = CleanupRef(kind="es_index", locator=self._EXPECTED_INDEX, owner_id="es_envelope.items_index")
        patch_ctx, _ = self._patch_es_client(raises=RuntimeError("conn refused"))
        with patch_ctx:
            outcome = await owner.cleanup_one(ref, CleanupMode.HARD)
        assert outcome == CleanupOutcome.RETRY
    ...
    def test_register_owners_adds_to_registry(self) -> None:
        ...
        reg = CascadeCleanupRegistry()
        register_owners(reg)
        assert reg.get(EsItemsEnvelopeIndexOwner.owner_id) is not None
