...
from dynastore.modules.storage.driver_config import CollectionDuckdbDriverConfig


class TestDuckDBDriverMeta:
    def test_driver_class_name(self):
        from dynastore.modules.storage.drivers.duckdb import CollectionDuckdbDriver
        driver = CollectionDuckdbDriver()
...
class TestDuckDBWritability:
    def test_is_writable_false_by_default(self):
        from dynastore.modules.storage.drivers.duckdb import CollectionDuckdbDriver
        driver = CollectionDuckdbDriver()
        loc = CollectionDuckdbDriverConfig(format="parquet", path="/data/f.parquet")
...
class TestDuckDBResolveLocation:
    ...
    async def test_resolve_returns_config(self):
        from dynastore.modules.storage.drivers.duckdb import CollectionDuckdbDriver
        driver = CollectionDuckdbDriver()
        loc = CollectionDuckdbDriverConfig(format="csv", path="/data/f.csv")
        with patch.object(driver, "_get_location_async", new_callable=AsyncMock, return_value=loc):
            result = await driver.resolve_storage_location("cat1", "col1")
            assert isinstance(result, CollectionDuckdbDriverConfig)
...
class TestDuckDBEnsureStorage:
    ...
    async def test_ensure_storage_file_not_found(self):
        from dynastore.modules.storage.drivers.duckdb import CollectionDuckdbDriver
        driver = CollectionDuckdbDriver()
        loc = CollectionDuckdbDriverConfig(format="parquet", path="/nonexistent/file.parquet")
        with patch.object(driver, "_get_location_async", new_callable=AsyncMock, return_value=loc):
