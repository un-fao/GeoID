"""Unit tests for ElasticsearchCollectionConfig + is_collection_obfuscated.
...
class TestResolverWaterfall:
    ...
    async def test_returns_false_when_no_configs_protocol(self, monkeypatch):
        ...
        assert await is_collection_obfuscated("cat", "col") is False
    ...
    async def test_collection_override_true_wins(self, monkeypatch):
        ...
        import dynastore.modules.elasticsearch.es_collection_config as mod
        proto = _fake_configs(
            collection_cfg=ElasticsearchCollectionConfig(obfuscated=True),
            catalog_cfg=ElasticsearchCatalogConfig(obfuscated=False),
        )
        monkeypatch.setattr(mod, "get_protocol", lambda p: proto)
        assert await is_collection_obfuscated("cat", "col1") is True
    ...
    async def test_collection_override_false_wins(self, monkeypatch):
        ...
        import dynastore.modules.elasticsearch.es_collection_config as mod
        proto = _fake_configs(
            collection_cfg=ElasticsearchCollectionConfig(obfuscated=False),
            catalog_cfg=ElasticsearchCatalogConfig(obfuscated=True),
        )
        monkeypatch.setattr(mod, "get_protocol", lambda p: proto)
        assert await is_collection_obfuscated("cat", "col2") is False

    @pytest.mark.asyncio
    async def test_inherits_catalog_when_collection_obfuscated_is_none(
        self, monkeypatch,
    ):
        import dynastore.modules.elasticsearch.es_collection_config as mod
        proto = _fake_configs(
            collection_cfg=ElasticsearchCollectionConfig(obfuscated=None),
            catalog_cfg=ElasticsearchCatalogConfig(obfuscated=True),
        )
        monkeypatch.setattr(mod, "get_protocol", lambda p: proto)
        assert await is_collection_obfuscated("cat", "col3") is True
    ...
    async def test_inherits_catalog_when_collection_cfg_absent(
        ...
        monkeypatch.setattr(mod, "get_protocol", lambda p: proto)
        assert await is_collection_obfuscated("cat", "col4") is True
    ...
    async def test_returns_false_when_no_configs_at_either_tier(
        ...
        monkeypatch.setattr(mod, "get_protocol", lambda p: proto)
        assert await is_collection_obfuscated("cat", "col5") is False
    ...
    async def test_collection_fetch_exception_falls_through_to_catalog(
        ...
        monkeypatch.setattr(mod, "get_protocol", lambda p: proto)
        assert await is_collection_obfuscated("cat", "col6") is True
    ...
    async def test_catalog_fetch_exception_returns_false(self, monkeypatch):
        ...
        monkeypatch.setattr(mod, "get_protocol", lambda p: proto)
        assert await is_collection_obfuscated("cat", "col7") is False
...
class TestResolverCache:
    ...
    async def test_repeated_call_hits_cache(self, monkeypatch):
        ...
        await is_collection_obfuscated("cat_cache_a", "col_cache_a")
        await is_collection_obfuscated("cat_cache_a", "col_cache_a")
    ...
    async def test_distinct_collection_keys_dont_collide(self, monkeypatch):
        ...
        await is_collection_obfuscated("cat_cache_b", "col_b1")
        await is_collection_obfuscated("cat_cache_b", "col_b2")
        await is_collection_obfuscated("cat_cache_b", "col_b1")  # cached
    ...
    async def test_apply_handler_invalidates_cached_entry(self, monkeypatch):
        ...
        # Round 1: collection_cfg has obfuscated=False — cache misses, then hits.
        proto1 = _fake_configs(
            collection_cfg=ElasticsearchCollectionConfig(obfuscated=False),
        )
        monkeypatch.setattr(mod, "get_protocol", lambda p: proto1)
        assert await is_collection_obfuscated("cat_inval", "col_inval") is False
        assert await is_collection_obfuscated("cat_inval", "col_inval") is False
        ...
        # Round 2: same proto returns the new obfuscated=True value;
        ...
        assert await is_collection_obfuscated("cat_inval", "col_inval") is True
