...
async def test_restricted_collection_post_only(apikey_manager):
    ...
    principal = Principal(
        provider="local",
        subject_id=f"restricted_{uuid4().hex[:8]}",
    ...
    restricted_policy = Policy(
        id=f"restricted_post_{uuid4().hex[:8]}",
        effect="allow",
        actions=["POST"],
        resources=[
...
async def test_item_level_get_access(apikey_manager):
    ...
    principal = Principal(
        provider="local",
        subject_id=f"item_reader_{uuid4().hex[:8]}",
        roles=[],
...
async def test_query_parameter_enforcement(apikey_manager):
    ...
    principal = Principal(
        provider="local",
        subject_id=f"query_enforced_{uuid4().hex[:8]}",
        roles=[],
...
async def test_deny_all_except_specific_operations(apikey_manager):
    ...
    principal = Principal(
        provider="local",
        subject_id=f"minimal_{uuid4().hex[:8]}",
        roles=[],
    ...
    get_policy = Policy(
        id=f"get_item_only_{uuid4().hex[:8]}",
        effect="allow",
        actions=["GET"],
        resources=[
            r"^/features/catalogs/demo_catalog/collections/demo_collection/items/[^/]+$"
...
async def test_cross_catalog_access_denial(apikey_manager):
    ...
    principal = Principal(
        provider="local",
        subject_id=f"single_catalog_{uuid4().hex[:8]}",
        roles=[],
    ...
    catalog_policy = Policy(
        id=f"demo_catalog_only_{uuid4().hex[:8]}",
        effect="allow",
...
async def test_no_list_operations_allowed(apikey_manager):
    ...
    principal = Principal(
        provider="local",
        subject_id=f"no_list_{uuid4().hex[:8]}",
        roles=[],
