...
@pytest.mark.asyncio
async def test_cancellation_mid_query_leaves_pool_clean(shared_engine):
    ...
    Note: this test attaches its own ``logging.Handler`` directly to the
    ``query_executor`` logger instead of relying on the pytest ``caplog``
    fixture, because some local invocation modes (e.g. ``-p no:logging``)
    disable the logging plugin and remove ``caplog`` from the fixture
    set, which would error this test at setup despite the assertions
    ...
    try:
        try:
            ...
            qe_logger.setLevel(_prev_level)
        ...
        qe_logger.removeHandler(capture)
