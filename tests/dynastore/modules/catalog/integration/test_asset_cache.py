...
async def test_asset_cache_targeted_invalidation(
    ...
    # Create two assets
    asset1 = AssetBase(
        asset_id=f"cache_test_1_{uuid.uuid4().hex[:8]}",
    ...
    asset2 = AssetBase(
        asset_id=f"cache_test_2_{uuid.uuid4().hex[:8]}",
...
async def test_asset_cache_bypassed_with_db_resource(
    ...
    # Create an asset
    asset = AssetBase(
        asset_id=f"txn_test_{uuid.uuid4().hex[:8]}",
        uri="gs://test/txn.tif",
        asset_type=AssetTypeEnum.RASTER,
        metadata={"value": 1},
    )
...
async def test_asset_cache_invalidation_on_delete(
    ...
    asset = AssetBase(
        asset_id=f"delete_test_{uuid.uuid4().hex[:8]}",
...
async def test_asset_cache_performance_improvement(
    ...
    for i in range(10):
        asset = AssetBase(
            asset_id=f"perf_test_{i}_{uuid.uuid4().hex[:8]}",
