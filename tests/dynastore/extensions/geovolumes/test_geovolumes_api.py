...
"""Unit tests for OGC API 3D GeoVolumes routes (Task 2.2).
...
@pytest.fixture()
def geovolumes_app():
    """Minimal FastAPI app with GeoVolumesService routes and mocked catalog service."""
    from dynastore.extensions.geovolumes.geovolumes_service import GeoVolumesService

    svc = GeoVolumesService()
...
# ---------------------------------------------------------------------------
# Task 2.2a — collections listing (3D filter)
# ---------------------------------------------------------------------------


def test_list_collections_returns_only_3d(geovolumes_app):
    app, _ = geovolumes_app
    r = TestClient(app).get(f"/geovolumes/catalogs/{_CAT}/collections")
...
def test_list_collections_has_collection_type(geovolumes_app):
    app, _ = geovolumes_app
    r = TestClient(app).get(f"/geovolumes/catalogs/{_CAT}/collections")
    assert r.status_code == 200
...
def test_list_collections_empty_when_no_3d(geovolumes_app):
    app, catalogs_mock = geovolumes_app
...
# ---------------------------------------------------------------------------
# Task 2.2b — single collection
# ---------------------------------------------------------------------------


def test_get_collection_returns_threedcontainer(geovolumes_app):
    app, _ = geovolumes_app
    r = TestClient(app).get(f"/geovolumes/catalogs/{_CAT}/collections/{_COL_3D}")
    assert r.status_code == 200
    data = r.json()
...
def test_get_collection_content_has_3dtiles_link(geovolumes_app):
    app, _ = geovolumes_app
    r = TestClient(app).get(f"/geovolumes/catalogs/{_CAT}/collections/{_COL_3D}")
    assert r.status_code == 200
...
def test_get_collection_content_has_cityjsonseq_link(geovolumes_app):
    app, _ = geovolumes_app
    r = TestClient(app).get(f"/geovolumes/catalogs/{_CAT}/collections/{_COL_3D}")
    assert r.status_code == 200
    content = r.json().get("content", [])
...
def test_get_collection_not_found_returns_404(geovolumes_app):
    app, catalogs_mock = geovolumes_app
    catalogs_mock.get_collection = AsyncMock(return_value=None)
    r = TestClient(app).get(f"/geovolumes/catalogs/{_CAT}/collections/missing")
    assert r.status_code == 404


def test_get_collection_non_3d_returns_404(geovolumes_app):
    app, catalogs_mock = geovolumes_app
    catalogs_mock.get_collection = AsyncMock(return_value=_COLLECTION_PLAIN)
    r = TestClient(app).get(f"/geovolumes/catalogs/{_CAT}/collections/{_COL_PLAIN}")
    assert r.status_code == 404


# ---------------------------------------------------------------------------
# Task 2.2c — cityjsonseq streaming
# ---------------------------------------------------------------------------


def test_cityjsonseq_streams_header_and_items(geovolumes_app):
    app, _ = geovolumes_app
    r = TestClient(app).get(
        f"/geovolumes/catalogs/{_CAT}/collections/{_COL_3D}/cityjsonseq"
    )
    assert r.status_code == 200
...
def test_cityjsonseq_content_type(geovolumes_app):
    app, _ = geovolumes_app
    r = TestClient(app).get(
        f"/geovolumes/catalogs/{_CAT}/collections/{_COL_3D}/cityjsonseq"
    )
...
def test_cityjsonseq_not_found_returns_404(geovolumes_app):
    app, catalogs_mock = geovolumes_app
    catalogs_mock.get_collection = AsyncMock(return_value=None)
    r = TestClient(app).get(
        f"/geovolumes/catalogs/{_CAT}/collections/missing/cityjsonseq"
    )
    assert r.status_code == 404


# ---------------------------------------------------------------------------
# Task 2.2d — bbox spatial filter on collections listing
# ---------------------------------------------------------------------------


def test_list_collections_bbox_4_filters(geovolumes_app):
    """A 4-number bbox that overlaps the 3D collection passes it through."""
    app, _ = geovolumes_app
    r = TestClient(app).get(
        f"/geovolumes/catalogs/{_CAT}/collections?bbox=4.0,52.0,4.5,52.2"
    )
    assert r.status_code == 200
...
def test_list_collections_bbox_excludes_non_overlapping(geovolumes_app):
    ...
    r = TestClient(app).get(
        f"/geovolumes/catalogs/{_CAT}/collections?bbox=10.0,10.0,11.0,11.0"
    )
    assert r.status_code == 200
    assert r.json()["collections"] == []


def test_list_collections_bbox_6_with_z_range_overlap(geovolumes_app):
    ...
    r = TestClient(app).get(
        f"/geovolumes/catalogs/{_CAT}/collections?bbox=4.0,52.0,10.0,4.5,52.2,30.0"
    )
    assert r.status_code == 200
    ids = [c["id"] for c in r.json()["collections"]]
    assert _COL_3D in ids
...
def test_list_collections_bbox_6_with_z_range_no_overlap(geovolumes_app):
    """A 6-number bbox with z range above the collection yields empty."""
    app, _ = geovolumes_app
    # zmin=100, zmax=200 — collection zmax is 50, so no z overlap
    r = TestClient(app).get(
        f"/geovolumes/catalogs/{_CAT}/collections?bbox=4.0,52.0,100.0,4.5,52.2,200.0"
    )
    assert r.status_code == 200
    assert r.json()["collections"] == []


def test_list_collections_malformed_bbox_returns_400(geovolumes_app):
    """A bbox with wrong arity (e.g. 3 numbers) returns HTTP 400."""
    app, _ = geovolumes_app
    r = TestClient(app).get(
        f"/geovolumes/catalogs/{_CAT}/collections?bbox=1.0,2.0,3.0"
    )
