...
"""Web-page registration smoke tests for the GeoVolumes globe browser.

Asserts that GeoVolumesService contributes:
- a web page with id ``geovolumes_browser`` via ``get_web_pages()``
- a static-asset prefix ``geovolumes`` via ``get_static_assets()``
...
MODULE_PATH = "dynastore.extensions.geovolumes.geovolumes_service"
CLASS_NAME = "GeoVolumesService"
PAGE_ID = "geovolumes_browser"
STATIC_PREFIX = "geovolumes"
HANDLER_NAME = "provide_geovolumes_browser"
...
def test_static_files_non_empty():
    ...
    for asset in svc.get_static_assets():
        if asset.prefix.strip("/") == STATIC_PREFIX:
            ...
            return
    pytest.fail("Static asset provider for prefix 'geovolumes' not found")
...
def _html_content() -> str:
    html_path = os.path.join(
        ...
        "..", "..", "..", "..",
        "packages", "extensions", "geovolumes", "src",
        "dynastore", "extensions", "geovolumes", "static",
        "geovolumes_browser.html",
    )
...
def test_html_includes_deckgl_script_with_integrity():
    ...
    # SRI integrity present (the maplibre tag already verified one exists; verify
    # the deck.gl tag also carries its own hash by counting integrity= occurrences)
    import re
...
def test_html_own_js_path():
    """Own JS is in the same directory as the HTML: ./geovolumes-globe.js."""
    html = _html_content()
    assert "./geovolumes-globe.js" in html, (
        "HTML does not reference geovolumes-globe.js via the correct ./ path"
    )
...
def test_html_no_unsubstituted_version_token():
    ...
    assert "{{VERSION}}" in html, (
        "geovolumes_browser.html is missing the {{VERSION}} template token"
    )
