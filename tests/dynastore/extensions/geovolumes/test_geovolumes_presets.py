...
"""Unit tests for GeoVolumesDemoPreset (Task 5.1)."""
...
def _make_ctx(
    *,
    ...
    # search_items returns a list of dummy items (emptiness probe)
    catalogs.search_items = AsyncMock(
        return_value=[MagicMock()] * items_count
    )
    # list_collections for catalog-emptiness probe
...
def _make_params(
    ...
    """Build a GeoVolumesDemoParams-shaped object."""
    from dynastore.extensions.geovolumes.presets import GeoVolumesDemoParams
...
def test_preset_name():
    from dynastore.extensions.geovolumes.presets import GEOVOLUMES_DEMO_PRESET
...
async def test_apply_creates_catalog_and_collection():
    ...
    from dynastore.extensions.geovolumes.presets import (
    ...
    with (
        patch(
            "dynastore.extensions.geovolumes.presets.ingest_cityjson_file",
            new_callable=AsyncMock,
            return_value={"items": 3, "failed": 0, "warnings": []},
        ) as mock_ingest,
        patch(
            "dynastore.extensions.geovolumes.presets._enqueue_tileset_task",
            new_callable=AsyncMock,
        ) as mock_enqueue,
    ):
...
@pytest.mark.asyncio
async def test_apply_enqueues_tileset_task():
    """apply() enqueues 'geovolumes_tileset' task after ingestion."""
    from dynastore.extensions.geovolumes.presets import (
        GEOVOLUMES_DEMO_PRESET,
        GeoVolumesDemoParams,
    )
    ...
    # first positional arg is the ctx, second is catalog_id, third is collection_id
    assert enqueue_args.args[1] == "demo-3d"
    assert enqueue_args.args[2] == "denhaag"
...
async def test_apply_downloads_when_no_local_path():
    ...
    with (
        patch(
            "dynastore.extensions.geovolumes.presets._download_source",
...
async def test_revoke_removes_what_preset_created():
    ...
    # Descriptor that says this preset created both
