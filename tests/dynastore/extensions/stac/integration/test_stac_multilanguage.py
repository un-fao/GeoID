...
async def test_create_catalog_with_language(in_process_client, catalog_id):
    ...
    r = await in_process_client.post("/stac/catalogs?lang=en", json=payload_en)
...
@pytest.mark.asyncio
@pytest.mark.enable_extensions("stac", "assets", "features", "configs", "logs")
async def test_add_french_translation_to_catalog(in_process_client, catalog_id):
...
@pytest.mark.asyncio
@pytest.mark.enable_extensions("stac", "assets", "features", "configs", "logs")
async def test_language_conflict_detection(in_process_client, catalog_id):
...
@pytest.mark.asyncio
@pytest.mark.enable_extensions("stac", "assets", "features", "configs", "logs")
async def test_accept_language_header_only(in_process_client, catalog_id):
...
@pytest.mark.asyncio
@pytest.mark.enable_extensions("stac", "assets", "features", "configs", "logs")
async def test_keywords_localization(in_process_client, catalog_id, collection_id):
    ...
    r = await in_process_client.post("/stac/catalogs?lang=en", json=payload)
    assert r.status_code == 201
