...
"""Registration + handler smoke tests for the per-extension web sections.

Each of STAC, OGC Features, and Assets contributes a navigation page and a
static-asset prefix via the ``@expose_web_page`` / ``@expose_static`` decorators.
These tests assert the contribution is discoverable (the page id and static
prefix surface through ``get_web_pages`` / ``get_static_assets``) and that the
page handler returns a 200 HTML response with the ``{{VERSION}}`` template token
substituted.

The services are instantiated via ``cls.__new__`` to avoid their full ``__init__``;
``collect_web_pages`` inspects the class MRO and binds methods with ``getattr``
(it never triggers ``@property`` descriptors), so a bare instance is sufficient.
"""
...
SECTIONS = [
    (
        "dynastore.extensions.stac.stac_service",
        "STACService",
        "stac_browser",
        "stac",
        "provide_stac_browser",
    ),
    (
        "dynastore.extensions.features.features_service",
        "OGCFeaturesService",
        "features_browser",
        "features",
        "provide_features_browser",
    ),
    (
        "dynastore.extensions.assets.assets_service",
        "AssetService",
        "assets_manager",
        "assets",
