...
def test_matrix_cloud_tasks_get_background() -> None:
    ...
    assert t2.consumers == ["catalog"]   # affinity_tier=None defaults to "catalog"
...
def test_matrix_cloud_processes_get_gcp_cloud_run() -> None:
    ...
    assert t.consumers == ["catalog"]    # ingestion not in CLOUD_PROCESS_CONSUMERS, defaults to ["catalog"]


def test_matrix_cloud_process_job_name() -> None:
    """Cloud preset: options dict is intentionally empty — job-name discovery is the runner's responsibility."""
...
def test_matrix_unknown_process_defaults_to_catalog_consumers() -> None:
    """Process not in CLOUD_PROCESS_CONSUMERS defaults to consumers=['catalog']."""
...
def test_resolved_targets_returns_process_bucket() -> None:
    ...
    target = RunnerTarget(consumers=["catalog"], runner="gcp_cloud_run", hints={ExecHint.OFFLOAD})
    cfg = TaskRoutingConfig(
        tasks={},
        processes={"ingestion": [target]},
    )
    result = cfg.resolved_targets("ingestion")
    assert result == [target]
...
def test_resolved_targets_unknown_key_returns_empty() -> None:
    ...
    result = cfg.resolved_targets("no_such_task_xyz")
    assert result == []
...
def _always_true(runner_type: str) -> bool:
    return True


def _never_true(runner_type: str) -> bool:
    return False


def test_select_target_superset_match() -> None:
    """Target with hints that are a superset of request_hints is chosen."""
    t1 = RunnerTarget(consumers=["catalog"], runner="background", hints={ExecHint.BACKGROUND})
    t2 = RunnerTarget(consumers=["catalog"], runner="gcp_cloud_run", hints={ExecHint.OFFLOAD, ExecHint.HEAVY})
...
def test_select_target_longest_hints_wins() -> None:
    ...
    t1 = RunnerTarget(consumers=["catalog"], runner="a", hints={ExecHint.BACKGROUND})
    t2 = RunnerTarget(consumers=["catalog"], runner="b", hints={ExecHint.BACKGROUND, ExecHint.SYNC})
...
def test_select_target_empty_request_first_viable() -> None:
    ...
    t1 = RunnerTarget(consumers=["catalog"], runner="a", hints={ExecHint.BACKGROUND})
    t2 = RunnerTarget(consumers=["catalog"], runner="b", hints={ExecHint.OFFLOAD})
...
def test_select_target_no_match_returns_none() -> None:
    ...
    t = RunnerTarget(consumers=["catalog"], runner="background", hints={ExecHint.BACKGROUND})
...
def test_select_target_wrong_service_filtered_out() -> None:
    ...
    t = RunnerTarget(consumers=["worker"], runner="background", hints={ExecHint.BACKGROUND})
...
def test_select_target_empty_target_hints_matches_all() -> None:
    ...
    t = RunnerTarget(consumers=["catalog"], runner="sync", hints=set())
