...
def register_maps_policies():
    """Register Maps public-access policy and anonymous role.
    ...
    _reg_policy(maps_policy)
    _reg_role(Role(name="anonymous", policies=["maps_public_access"], is_system=True))

    logger.debug("Maps policies pre-registered into in-memory registry.")

    # If PermissionProtocol is already live, push immediately.
    policy_manager = get_protocol(PermissionProtocol)
    if policy_manager:
        policy_manager.register_policy(maps_policy)
        policy_manager.register_role(
            Role(name="anonymous", policies=["maps_public_access"], is_system=True)
        )
        logger.debug("Maps policies also applied to live PermissionProtocol.")
