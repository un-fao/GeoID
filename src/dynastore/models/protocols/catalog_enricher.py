...
"""
CatalogEnricherProtocol — optional pipeline for enriching or filtering
catalog-level metadata at read time.

Enrichers are registered via ``register_plugin(enricher)`` and discovered
with ``get_protocols(CatalogEnricherProtocol)``.  The pipeline in
``CatalogService.get_catalog_model()`` runs all enrichers in ascending
...
Typical use cases:
  ...
  - Inject computed summaries (e.g. storage usage from BigQuery).
"""
...
@runtime_checkable
class CatalogEnricherProtocol(Protocol):
    """Pipeline stage for enriching or filtering catalog metadata.
    ...
    that controls whether the enricher applies to a given catalog.
    ...
    Attributes:
        enricher_id: Unique string identifier (e.g. ``"bq_catalog_enricher"``).
    ...
    def can_enrich(self, catalog_id: str) -> bool:
        """Return ``True`` if this enricher applies to the given catalog.

        Called before ``enrich()`` to skip enrichers that are not relevant
        (e.g. enrichers tied to a specific catalog or driver type).

        Args:
            catalog_id: The catalog whose metadata is being read.
    ...
    async def enrich(
        ...
        """Enrich or filter catalog metadata.
        ...
        Args:
            catalog_id: The catalog being enriched.
