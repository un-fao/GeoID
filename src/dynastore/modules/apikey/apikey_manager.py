...
# File: dynastore/modules/apikey/apikey_manager.py
...
class ApiKeyManager(ApiKeyProtocol):
    """
    ...
    def __init__(
        ...
        policy_manager: Optional[object] = None,
    ...
    async def get_jwt_secret(self) -> str:
        ...
        # 1. Check PropertiesProtocol (Standardized IAG v2.1)
        ...
        # 2. Fallback to storage or env
        # secret = await self.storage.get_setting("jwt_secret")
        # if not secret:
        secret = os.environ.get("JWT_SECRET", secrets.token_urlsafe(32))
        # Persist the newly generated secret if PropertiesProtocol is available
        if props:
            await props.set_property("apikey_jwt_secret", secret, "apikey_extension")
        else:
            logger.warning(
                "PropertiesProtocol not available. Cannot persist new JWT secret."
            )
        # await self.storage.set_setting("jwt_secret", secret) # Also persist in storage for backward compatibility/fallback
    ...
    async def authenticate_apikey(
        ...
        if metadata.policy:
            ...
            is_allowed = await self.policy_manager.evaluate_policy_statements(
