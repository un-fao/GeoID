...
class PlatformConfigManager(ProtocolPlugin[object], PlatformConfigsProtocol):
    ...
    async def lifespan(self, app_state: Any) -> typing.AsyncGenerator[None, None]:
        ...
        logger.info("PlatformConfigManager: Started.")
        yield
        logger.info("PlatformConfigManager: Stopped.")
    ...
    async def initialize_storage(cls, conn: DbResource):
        ...
        try:
            ...
            logger.error(
                f"FATAL: PlatformConfigManager initialization failed: {e}",
                exc_info=True,
            )
            raise
