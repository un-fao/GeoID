...
@cached(maxsize=8192, ttl=300, namespace="catalog_config_mgr", ignore=["engine", "catalog_manager"])
...
@cached(maxsize=16384, ttl=300, namespace="collection_config_mgr", ignore=["engine", "catalog_manager"])
...
class ConfigManager(ConfigsProtocol):
    ...
    def __init__(
        ...
        self._platform_config_manager = platform_config_manager
    ...
    def catalog_manager(self, value: CatalogsProtocol):
        self._catalog_manager = value

    @property
    def platform_config_manager(self) -> PlatformConfigsProtocol:
        return self._get_platform_config_manager()

    @platform_config_manager.setter
    def platform_config_manager(self, value: PlatformConfigsProtocol):
        self._platform_config_manager = value
    ...
    def _get_catalog_manager(self) -> CatalogsProtocol:
        if self._catalog_manager is None:
            ...
            self._catalog_manager = get_protocol(CatalogsProtocol)
        return self._catalog_manager

    def _get_platform_config_manager(self) -> PlatformConfigsProtocol:
        if self._platform_config_manager is None:
            from dynastore.tools.discovery import get_protocol
            self._platform_config_manager = get_protocol(PlatformConfigsProtocol)
        if self._platform_config_manager is None:
            # Fallback for late initialization
            from dynastore.modules.db_config.platform_config_manager import PlatformConfigManager
            self._platform_config_manager = PlatformConfigManager()
        return self._platform_config_manager
    ...
    async def get_config(
        ...
        return await self._get_platform_config_manager().get_config(
    ...
    async def set_config(
        ...
        if collection_id is not None:
            ...
            # Platform level
            await (await self._get_platform_config_manager()).set_config(
                plugin_id,
                config,
                check_immutability=check_immutability,
                db_resource=db_resource,
    ...
    async def set_platform_config(self, plugin_id: str, config: PluginConfig) -> None:
        await self._get_platform_config_manager().set_config(plugin_id, config)
    ...
    async def list_configs(
        ...
        if collection_id is not None:
            ...
            configs = await self._get_platform_config_manager().list_configs()
    ...
    async def delete_config(
        ...
        if collection_id is not None:
            ...
            await self.platform_config_manager.delete_config(
                plugin_id, db_resource=db_resource
            )
