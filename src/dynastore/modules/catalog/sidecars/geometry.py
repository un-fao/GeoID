...
class GeometrySidecar(SidecarProtocol):
    ...
    def provides_feature_id(self) -> bool:
        ...
        return False  # Geometry sidecar depends on geoid, it doesn't generate it.
    ...
    def get_select_fields(
        ...
        alias = sidecar_alias or "g"

        # We prefer GeoJSON format for the stream unless a raw path is requested.
        # ST_AsGeoJSON output is cast to jsonb to match the O(1) streaming consumer.
        fields = [f"ST_AsGeoJSON({alias}.{self.config.geom_column})::jsonb as geom"]

        if self.config.bbox_column:
            fields.append(f"ST_AsGeoJSON({alias}.{self.config.bbox_column})::jsonb as bbox_geom")
    ...
    def prepare_upsert_payload(
        ...
        if self.config.write_bbox:
            if geom_data.get("bbox_coords"):
                ...
                payload["bbox_geom"] = wkb.dumps(
                    box(*geom_data["bbox_coords"]), hex=True
                )
            elif wkb_hex:
                bbox_val = self._get_val(feature, "bbox_geom")
                if bbox_val:  # Optimization: if passed directly
                    payload["bbox_geom"] = bbox_val  # Expecting WKB Hex
    ...
    def get_internal_columns(self) -> set:
        ...
        return {
            "geom", "bbox_geom", "centroid",
            "area", "length", "volume",
            "was_geom_fixed", "recalculated_geom",
            "geom_stats",
        }
    ...
    def map_row_to_feature(
        ...
        Also stores all geometry-sidecar row values in
        ``context["_sidecar_data"]["geometry"]`` for downstream sidecars.
        """
        ...
        sidecar_data[self.sidecar_id] = {
            k: row[k] for k in self.get_internal_columns() if k in row
        }
        # 1. Geometry Mapping
        geom_col = self.config.geom_column
        if geom_col in row:
            geom_data = row[geom_col]
        ...
        # 2. BBox Mapping
        if self.config.bbox_column and self.config.bbox_column in row:
            # We could extract the bbox array from box geometry if needed.
            # For now we just ensure it's published in context for downstream sidecars.
            pass

        # 2. BBox Mapping
        if "bbox_geom" in row:
            # bbox_geom is often a list/tuple from PostGIS (box2d/box3d conversion)
            # or it might need parsing from a string
            bbox_data = row["bbox_geom"]
            if isinstance(bbox_data, (list, tuple)):
                feature.bbox = list(bbox_data)
            elif isinstance(bbox_data, str):
                try:
                    # Generic parser for 'BOX(minx miny, maxx maxy)'
                    import re
                    match = re.search(r"BOX\(([-\d.]+) ([-\d.]+),\s*([-\d.]+) ([-\d.]+)\)", bbox_data)
                    if match:
                        feature.bbox = [float(x) for x in match.groups()]
                except Exception:
                    pass

        # JSON-FG: Place and coordRefSys Mapping
        if "place" in row:
            place_data = row["place"]
            from pydantic import TypeAdapter
            
            if isinstance(place_data, dict):
                feature.place = TypeAdapter(Geometry).validate_python(place_data)
            elif isinstance(place_data, str):
                import json
                feature.place = TypeAdapter(Geometry).validate_json(place_data)
                
        if "coordRefSys" in row:
            feature.coordRefSys = row["coordRefSys"]
    ...
    def apply_query_context(
        ...
        """
        # Search for spatial operators in filters
        for f in request.filters:
            if f.spatial_op:
                # Signal that this sidecar handles these spatial filters
                # The QueryOptimizer will use this information.
                pass
        pass
